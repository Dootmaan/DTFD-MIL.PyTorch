...
slides_folder_dir = '/newdata/why/CAMELYON16/training/tumor/'
slide_paths = glob.glob(os.path.join(slides_folder_dir, '*.tif'))  # change the surfix '.tif' to other if necessary
save_folder_dir = '/newdata/why/CAMELYON16/extracted_patches/training/tumor/'
####======================================
                ...
                if mRatio > threshold:
                    for sstLevel, tSize in zip(level_list, patch_size_list):
                        try:
                            ...
                            tpatch.convert("RGB").save(os.path.join(tsave_folder, tname))
                        except:
