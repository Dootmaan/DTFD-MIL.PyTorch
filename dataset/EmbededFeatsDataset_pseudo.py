...
class EmbededFeatsDataset(torch.utils.data.Dataset):
    def __init__(self,path,mode='train'):
        ...
        if mode=='train' or self.mode=='val':
            filenames=sorted(glob.glob(path+'/extracted_patches/training/*/256.1/*/feats_bag5*.npy'))
            random.seed(552) # make sure each time we have the same filenames order.
            random.shuffle(filenames)
            random.seed()
            train_frac, val_frac = 0.02, 0.2
        ...
        if mode=='test':
            filenames=sorted(glob.glob(path+'/extracted_patches/testing/*/256.0/*/feats_bag5*.npy'))[:5]
